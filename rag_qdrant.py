...
- Guide the user through each verification step"""
    ...
    def ingest_text_chunks(self, chunks: List[str], source: str = "unknown"):
...
def main():
    ...
    try:
        ...
        recreate_collections = "--recreate" in sys.argv
        if recreate_collections:
            print("\n🔄 Recreating collections...")
            sys.argv.remove("--recreate")  # Remove the flag to avoid interfering with other args
        ...
        if len(sys.argv) >= 3 and sys.argv[1] == "ingest_pdf":
            ...
            rag.verify_collections()  # Verify after ingestion
            return
        ...
        # Get device type and phone type from user
        rag.device_type, rag.phone_type = get_valid_device_type()
