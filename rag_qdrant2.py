...
class AgenticRAG:
    def __init__(self, products_csv_path: str = "products.csv"):
        ...
        self.qdrant_client = QdrantClient(url="http://localhost:6333", api_key=self.qdrant_api_key)
    ...
    def _ensure_collections_exist(self):
        """Ensure both knowledge and context collections exist in Qdrant"""
        try:
            # Get embedding dimension (OpenAI text-embedding-3-small is 1536 dimensions)
            vector_size = 1536
            
            # Check and create knowledge collection if it doesn't exist
            collections = self.qdrant_client.get_collections().collections
            existing_collections = [col.name for col in collections]
            
            if self.knowledge_collection not in existing_collections:
                print(f"Creating collection: {self.knowledge_collection}")
            ...
            if self.context_collection not in existing_collections:
                print(f"Creating collection: {self.context_collection}")
                self.qdrant_client.create_collection(
                    collection_name=self.context_collection,
                    vectors_config=rest.VectorParams(
                        size=vector_size,
    ...
    def get_relevant_context(self, query: str, limit: int = 3) -> List[Dict]:
        """Get relevant context with error handling for empty collections"""
        try:
            # Check if collection has any documents first
            collection_info = self.qdrant_client.get_collection(self.context_collection)
            if collection_info.points_count == 0:
                print("Context collection is empty, returning empty context")
                return []
    ...
    def save_context(self, user_input: str, response: str, metadata: Dict = None):
        ...
        try:
            ...
            self.vectorstore_context.add_documents([context_doc])
        except Exception as e:
            print(f"Warning: Error saving context: {e}")
...
""",
    ...
    def clean_none_content_documents(self, batch_size: int = 100) -> int:
        ...
        try:
            ...
            print(f"Warning: Error cleaning documents: {e}")
            return 0